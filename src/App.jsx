...
const formatBangkokDateTimeLocalValue = (dateStr) => {
  ...
  const bangkokWallClock = shiftToBangkokWallClock(date);
  const year = bangkokWallClock.getUTCFullYear();
  const month = String(bangkokWallClock.getUTCMonth() + 1).padStart(2, '0');
  const day = String(bangkokWallClock.getUTCDate()).padStart(2, '0');
  const hour = String(bangkokWallClock.getUTCHours()).padStart(2, '0');
  const minute = String(bangkokWallClock.getUTCMinutes()).padStart(2, '0');
  return `${year}-${month}-${day}T${hour}:${minute}`;
};

const formatBangkokHintDate = (dateStr) => {
...
const readStoredRoleData = () => {
  try {
    const rawValue = localStorage.getItem(AUTH_STORAGE_KEY);
    if (!rawValue) return null;
    const parsedValue = JSON.parse(rawValue);
    if (!parsedValue || !parsedValue.role || !parsedValue.id || !parsedValue.name) return null;
    return parsedValue;
  } catch {
    return null;
  }
...
const writeStoredRoleData = (roleData) => {
  try {
    if (roleData) {
      localStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify(roleData));
    } else {
...
const createDefaultHintDate = (hintIndex, referenceDate = new Date()) => {
  ...
  const bangkokWallClock = shiftToBangkokWallClock(referenceDate);
  bangkokWallClock.setUTCDate(bangkokWallClock.getUTCDate() + offset);
  return bangkokWallClockToIso(bangkokWallClock);
};
...
const createNextHintDate = (existingSchedule = []) => {
  ...
  if (lastDateString) {
    const lastDate = shiftToBangkokWallClock(new Date(lastDateString));
...
export default function App() {
  const [showLanding, setShowLanding] = useState(() => !readStoredRoleData());
  const [user, setUser] = useState(null);
  const [roleData, setRoleData] = useState(() => readStoredRoleData());
  ...
  useEffect(() => {
    ...
    void updateDoc(docRef, {
      ...
      [`juniorsInfo.${roleData.id}`]: displayName,
    }).catch(console.error);
  }, [roleData, gameState]);

  const handleLogin = (role, id, name, nickname = '') => {
    const nextRoleData = { role, id, name, nickname };
    setRoleData(nextRoleData);
    setShowLanding(false);
    writeStoredRoleData(nextRoleData);
  ...
  const handleLogout = async () => {
    try {
      if (roleData?.role === 'junior' && gameState) {
         ...
         await updateDoc(docRef, { juniorsOnline: updatedOnline });
      }
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans relative selection:bg-white/30 selection:text-white" style={{ overflowX: 'hidden' }}>
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
          ...
          <nav className="fixed top-0 w-full z-50 px-3 sm:px-4 md:px-6 py-3 sm:py-4 transition-all duration-300 animate-slide-down">
            <div className="max-w-7xl mx-auto bg-white/10 backdrop-blur-2xl border border-white/10 rounded-[1.5rem] sm:rounded-full px-4 sm:px-6 py-3 flex justify-between items-center gap-3 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] min-w-0">
              ...
              <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                <div className="px-3 sm:px-4 py-1.5 rounded-full bg-black/20 backdrop-blur-md border border-white/5 text-xs flex items-center gap-2 font-medium max-w-[55vw] sm:max-w-none truncate">
                ...
                <button onClick={handleLogout} className="min-h-touch min-w-touch w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-all backdrop-blur-md border border-white/10 text-white/70 hover:text-white group">
        ...
        <main className={`flex-1 flex flex-col min-w-0 ${!showLanding && roleData ? 'pt-24 sm:pt-28 pb-10 sm:pb-12 px-3 sm:px-4 md:px-8 max-w-7xl mx-auto w-full' : 'w-full'}`}>
          {showLanding ? (
    ...
    >
       ...
       {/* 2. HERO SECTION: Liquid Card & Canva */}
        <div className="w-full max-w-5xl flex flex-col items-center gap-6 sm:gap-10 relative z-10 px-3 sm:px-4">
          ...
          <button 
            onClick={onEnter}
            className="mt-4 mb-14 sm:mb-20 px-8 sm:px-10 py-3 sm:py-4 bg-white/90 backdrop-blur-xl text-black font-bold text-base sm:text-lg rounded-full hover:bg-white hover:scale-105 transition-all duration-300 shadow-[0_0_40px_rgba(255,255,255,0.3)] flex items-center justify-center gap-2 group opacity-0 animate-slide-up min-h-touch"
...
function LoginScreen({ onLogin, onBack }) {
  ...
  return (
    <div className="flex-1 flex items-center justify-center w-full min-h-[80vh]">
      <div className="bg-white/10 backdrop-blur-2xl border border-white/10 p-6 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] w-full max-w-md shadow-[0_32px_64px_rgba(0,0,0,0.5)] relative overflow-hidden animate-scale-in min-w-0">
...
function AdminDashboard({ gameState, appId }) {
  ...
  const handleDateChange = (index, value) => {
    ...
    updatedSchedule[index] = parseBangkokDateTimeLocalToIso(value) || createDefaultHintDate(index);
  ...
  return (
    <div className="flex flex-col gap-8 w-full min-w-0">
      <div className="flex justify-center w-full animate-slide-down">
        <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-1 rounded-full flex gap-1 shadow-2xl overflow-x-auto w-full max-w-lg min-w-0">
          {[
      ...
      <div className="animate-fade-in w-full flex flex-col items-center min-w-0">
        {activeTab === 'live' ? (
          <div className="flex flex-col items-center justify-center w-full">
            <div className="text-center mb-10 bg-white/5 backdrop-blur-2xl border border-white/10 p-6 sm:p-8 rounded-[2rem] w-full max-w-3xl shadow-xl animate-scale-in min-w-0">
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 tracking-tight transition-all">
            ...
            {!isDrawStageOpen ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 w-full max-w-4xl min-w-0">
                {JUNIORS.map((j, idx) => {
          ...
          <div className="bg-white/10 backdrop-blur-3xl border border-white/10 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 md:p-10 max-w-5xl mx-auto w-full shadow-2xl animate-scale-in min-w-0">
            <div className="mb-8">
          ...
          <div className="bg-white/10 backdrop-blur-3xl border border-white/10 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 md:p-10 max-w-xl mx-auto w-full shadow-2xl animate-scale-in min-w-0">
            ...
            <div className="space-y-5">
               {hintSchedule.map((dateStr, idx) => {
                 const localDateStr = formatBangkokDateTimeLocalValue(dateStr);
                 return (
        ...
        {editingCard && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 z-50 animate-fade-in">
...
function SeniorDashboard({ gameState, appId, seniorId }) {
  ...
  return (
    <div className="max-w-xl mx-auto w-full animate-scale-in min-w-0">
      <div className="bg-white/10 backdrop-blur-3xl border border-white/10 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 md:p-10 shadow-2xl relative overflow-hidden min-w-0">
